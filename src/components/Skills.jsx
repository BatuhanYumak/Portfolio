  ...
  return (
    <section id="skills" className="py-24 bg-[#FAFAFA]">
      <div className="max-w-5xl mx-auto px-6">
        ...
        >
          <h2 className="text-2xl font-bold text-[#0D1B2A] mb-3">Skills</h2>
          <div className="w-10 h-[3px] bg-[#0D1B2A] mb-12" />

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {SKILL_COLUMNS.map(({ category, items }) => (
              <div key={category}>
                <p className="text-[11px] font-medium uppercase tracking-[0.1em] text-[#0D1B2A] mb-4">
                  {category}
                </p>
                <div className="flex flex-wrap gap-2">
                  {items.map(item => (
                    <span
                      key={item}
                      className="px-4 py-2 text-[14px] font-medium text-[#0D1B2A] border border-[#E5E7EB] rounded-full bg-white"
                    >
                      {item}
                    </span>
                  ))}
